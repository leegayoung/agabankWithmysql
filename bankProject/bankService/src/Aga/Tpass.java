	...
	public Tpass(){
		...
		ipbt.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e) {
		    	...
		    	ds.setPassword(Integer.parseInt(ta.getText()));
		    	int value = Integer.parseInt(ta.getText());
		    	ds.setPassword(value);
		    	
		    	
		    	if(dw.match_Password(ds.getMyacc(),ds.getPassword()))
		    	{
		    		mainf.revalidate();
			    	mainf.dispose();
			    	dw.balancecheck(ds.getMyacc());
			    	for(int i =0; i<5; i++)
			    		System.out.println(ds.info[i]);
					...
					Messege m = new Messege("비밀번호가 맞지않습니다.");
		    	}
