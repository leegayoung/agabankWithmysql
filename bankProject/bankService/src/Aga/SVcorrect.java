...
public class SVcorrect{
	...
	public SVcorrect(){
		...
		JLabel acc = new JLabel("* 계좌번호 ");
		acc.setForeground(new Color(40,40,40));
		acc.setFont(font2);
		...
		text.setLocation(140,200);
		
		JButton image = new JButton(new ImageIcon("image2/lock.png"));
		image.setLocation(400,100);
		image.setSize(64,64);
		...
		JLabel title1 = new JLabel("[ 정보 수정 ]");
		title1.setFont(font2);
		title1.setForeground(new Color(40,40,40));
		
		JLabel title = new JLabel("을 위해 본인 확인 해주세요.  ");
		title.setSize(500,50);
		title.setFont(font2);
		title.setForeground(Color.gray);
		title.setLocation(190,150);
		...
		ipbt.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e) {
		    	...
		    	ds.setMyacc(acc1.getText());
		    	ds.setSsNumber(number1.getText());
		    	System.out.println(acc1.getText() +"  "+number1.getText() );
		    	System.out.println(" dffd "+dw.Check_Acc(acc1.getText()));
		    	System.out.println(" dffd2 "+dw.Check_ssNumber(number1.getText()));
		    	if(dw.Check_Acc(acc1.getText()) && dw.Check_ssNumber(number1.getText()) )
		    	{
		    		System.out.println("dudd");
		    		String name1 = dw.return_Name_Acc_Info(ds.getMyacc());
		    		String name2 = dw.return_Name_User_Info(ds.getSsNumber());
		    		System.out.println(name1 + name2);
		    		if(name1.equals(name2))
		    		{
		    			SVupdate SVupdate = new SVupdate();
		    		}
		    		else{
