...
//				JTextField mlimit1 = new JTextField();
		    	...
		    	ds.setName(name);
		    	if(dw.return_Name_User_Info(num).equals(name))
		    	{
		    		dw.createAcc(name, num, phone, inputMoney,  Integer.parseInt(password), 0, mlimit, limit);
					//여기서 랜덤으로 계좌 생성
		    		//계좌번호
		    		//계좌번호 띄워주는 창이야
			    //	Messege m = new Messege(ds.getName()+"님의 계좌번호는 "+ds.getMyacc()+"입니다.");
		    		Messege m = new Messege("계좌번호는 "+ds.getMyacc());
		    	}else{
		    		//오류메세지 띄우기
		    		//계좌번호 띄워주는 창이야
			    	Messege m = new Messege("회원정보에서 등록을 해주세요.");
