...
public class Nmynum{
	...
	public Nmynum(){
		...
		JLabel textl = new JLabel("본인의 주민등록번호를 입력해주세요.");
		textl.setFont(font2);
		...
		ipbt.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e) {
		    	...
		    	ds.setSsNumber(ta.getText());
		    	if(dw.Check_ssNumber(ds.getSsNumber()))
		    	{
		    		ds.setName(dw.return_Name_User_Info(ds.getSsNumber()));
		    		Nyounum Nyouname= new Nyounum();
		    	}
		    	else 
		    	{
		    		AgaMain Omoney= new AgaMain();
					Messege m = new Messege("주민등록번호가 맞지않습니다.");
		    	}
